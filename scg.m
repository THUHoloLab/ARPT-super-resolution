function [x, options, flog, pointlog, scalelog,dataTerm, priorTerm] = scg(f, x, options, gradf, varargin)
...
sigma0 = 1.0e-4;
[fold ,dataTerm, priorTerm]= feval(f, x, varargin{:});	% Initial function value.
fnow = fold;
...
while (j <= niters)
  ...
  xnew = x + alpha*d;
  [fnew ,dataTerm, priorTerm] = feval(f, xnew, varargin{:});
